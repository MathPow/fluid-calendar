...
function buildPrompt(transcript: string, title?: string): string {
  ...
  return [
    ...
    "Summarize the transcript below. Reply in the SAME language as the transcript.",
    "Use concise Markdown with these sections (omit a section if empty):",
    "## TL;DR — 1-2 sentences",
...
export async function summarizeTranscript(
  ...
  try {
    res = await fetch(`${OLLAMA_URL}/api/generate`, {
      ...
      body: JSON.stringify({
        model: OLLAMA_MODEL,
        prompt: buildPrompt(transcript, opts?.title),
        stream: false,
